...
* ``<TRACKER_HASH>`` - Cookie hash generated by JS tracker. Check :ref:`how to get cookie hash<howToGetTrackerCookieHash>` section for detailed information.
...
.. _howToGetTrackerCookieHash:

How to get tracker cookie hash
------------------------------
If there is no non-AMP page tracked by traditional JS tracker, this value may be removed from configuration or left empty. It's used to guarantee that same cookie will be used by AMP and non-AMP pages on client domain. This value should be taken from the name of the ID cookie generated by JS tracker. Each JS tracker generates unique cookie name based on its configuration. Follow these instructions to get hash from cookie generated by JS tracker:

* Setup JS tracker on non-AMP page (if it was not done already).
* Open tracked page in the browser.
* Open developer tools in the browser and look for cookie starting with ``_pk_id.``. Cookie name should look similar to this: ``_pk_id.12345678-1234-1234-1234-1234567890ab.cdef``. The part after first dot is the value of App ID of the cookie (if there are multiple cookies starting with ``_pk_id.`` it may be used to identify correct cookie). After second dot you'll find the cookie hash generated by JS tracker (in the example its value is ``cdef``). Copy this part and replace ``<TRACKER_HASH>`` with it.

Here you can see how to look for tracker cookie in Google Chrome developer tools:
